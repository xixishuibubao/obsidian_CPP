...
**非完全模块化**下的架构：**接口与实现物理分离** + **单向分层依赖** + **Pimpl/纯虚接口/依赖注入** 隔离不可避免耦合；工具链（CMake、clang-tidy、Graphviz）固化规则。

### 核心要点清单

- 分层：`main` → 业务模块 → 核心库 → 基础组件（禁止反向依赖）
- 对外仅 `public/*.h` + 工厂函数；内部依赖放 `detail/`、`src/`
- 解耦五法：接口隔离、依赖注入、Pimpl、事件总线、SemVer 链接
- 演进三阶段：应急 Pimpl → 接口重构 → CI 依赖检查
- 参考：Chromium `net/public` vs `internal/`

### 交叉引用

- [[D-系统与架构/02.软件架构设计/1. PIMPL设计|PIMPL 具体写法]]
- [[D-系统与架构/02.软件架构设计/5. 插件架构设计|插件与工厂模式]]
- [[B-构建与脚本/01.构建工具/3. CMake入门与实践|CMake 依赖声明]]
- [[G-语言与标记/4. DOT与PlantUML绘图|DOT / PlantUML 绘图]]

## 【完整拓展详情】


## 1.1 架构设计核心原则（应对非完全模块化）
...
### 1.3.1 接口隔离（Interface Isolation）
...
### 1.3.5 版本化依赖（Semantic Versioning）
...
语法与图类型选型见 [[G-语言与标记/4. DOT与PlantUML绘图|DOT 与 PlantUML 绘图]]。

### 1.4.3 编译时间优化（Precompiled Headers）
...
### 1.5.1 阶段1：应急解耦（快速处理现有依赖）

- 使用Pimpl隐藏实现依赖
- 为不可避免的依赖添加`// NOLINT(deprecated)`注释标记

### 1.5.2 阶段2：接口重构（3-6个月）
...
1. 所有模块通过`net::URLRequest`抽象接口通信
2. 平台相关代码隔离在`internal/`目录，通过编译宏选择
3. 禁止`api/`目录依赖`internal/`，确保跨平台一致性

## 1.7 总结：非完全模块化的架构设计

1. **接口优先**：通过纯虚类定义模块契约，实现与实现解耦
2. **分层控制**：强制单向依赖，核心模块下沉（如基础库→核心库→业务模块）
3. **依赖可视化**：使用工具链（CMake+Graphviz）实时监控依赖关系
4. **渐进式重构**：从应急解耦到接口重构，最终固化架构规则
