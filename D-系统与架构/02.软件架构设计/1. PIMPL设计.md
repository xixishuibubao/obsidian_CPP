...
**PIMPL（Pointer to IMPL）**：头文件只暴露接口 + 不透明指针/前向声明，实现细节放 `.cpp` 或 `detail/`，减少编译依赖与 ABI 暴露。Qt 私有实现、`std::unique_ptr<Impl>` 是主流写法。

### 核心要点清单

- 嵌套命名空间：`struct Module::Impl` 与类同 namespace
- 匿名 namespace + `void*`：C 风格不透明指针，`static_cast` 在 .cpp 内完成
- `unique_ptr<detail::XxxImpl>` + `using Impl =`：现代 C++ 推荐
- 模板 `OpaquePtr<T>`：减少重复 `static_cast`
- `operator->` 代理：多层 `->` 穿透，头文件零类型信息
- 静态成员 / 函数指针：避免在头文件导出符号

### 方案选型

| 场景 | 写法 |
|------|------|
| 常规模块 | `unique_ptr` + 嵌套 `struct Impl` |
| 极致隐藏 | 匿名 namespace + `void*` |
| 多 impl 复用 | 模板基类 `PimplBase<Derived>` |

### 交叉引用

- [[D-系统与架构/02.软件架构设计/2. 结构化设计|结构化设计与编译防火墙]]
- [[D-系统与架构/02.软件架构设计/5. 插件架构设计|插件接口隐藏]]
