...
GC 算法谱系：**引用计数**（简单、怕循环引用）→ **标记-清除/整理**（Redis/V8）→ **复制/分代**（HotSpot/Go 思路）。C++ 工程常用 **RAII + 智能指针**，GC 库（Booster/folly）多用于脚本绑定或特定子系统。

### 核心要点清单

- 引用计数：`AddRef/Release`，循环引用需 **mark-sweep 周期检测** 或 `weak_ptr`
- 标记-清除：从根集合遍历；STW 停顿是工程痛点
- 分代 + 写屏障：Young 频繁回收，Old 少回收
- 学习顺序：tiny-gc-cpp → Booster.GC → V8 mark-compact 文档
- 调试：Valgrind Helgrind、ASan、Chrome DevTools GC 面板

### 章节地图（详情区）

- 1.1 引用计数入门 · 1.2 标记-清除/复制 · 1.3 开源 GC 库 · 1.4–1.6 书籍/路径/工具
- 1.7 汇编优化分配 · 1.8 分配器项目 · 1.9 C/C++ 底层实现路线

### 交叉引用

- [[D-系统与架构/03.并发与内存/2. atomic与内存序|atomic 与屏障]]
- [[A-编程语言/02.C++/4. C++现代特性|智能指针与 RAII]]

## 【完整拓展详情】

## 1.1 入门篇：最小化 GC 实现（引用计数）
...
public:
    ...
    void Collect() {
        ...
        memcpy(to, from, from_idx_);  // 复制存活对象到 to-space
        to_idx_ = from_idx_;          // 记录 to-space 已用大小
        from_idx_ = 0;                // 重置 from-space
        std::swap(from_idx_, to_idx_); // 交换空间角色
    }
};
...
- **视频**：
  - [CppCon 2019 《Writing Lock-Free Code: A Practical Guide》](https://www.youtube.com/watch?v=Y4GqK4r18KY)（含汇编优化案例）
...
1. **避免过度优化**：先用 C 实现功能，再通过 `perf` 定位热点（如 90% 时间在空闲块查找）。
2. **平台兼容性**：x86 汇编优化需适配不同 CPU（如 Intel vs AMD 的 `cmpxchg` 延迟差异）。
3. **内存屏障慎用**：优先使用编译器内置的 `__ATOMIC_*` 语义，仅在必要时插入汇编屏障。

通过以上资源，你将掌握如何在内存分配的关键路径（如无锁操作、内存对齐）中精准使用汇编，在保持代码可维护性的同时，突破纯 C 实现的性能瓶颈。建议从修改 `ptmalloc` 的小对象分配路径开始，逐步积累汇编优化经验。

## 1.8 内存分配器设计

学习内存分配器设计的项目推荐与学习路径，结合工业级实现和教学案例，从基础到进阶掌握内存管理核心技术。

### 1.8.1 必学的工业级内存分配器项目

#### 1.8.1.1 Google TCMalloc
...
针对 **C/C++ 底层实现（内存分配、资源管理、IO、锁）** 的系统化学习路线，结合源码分析、调试实践和经典资源，适合有一定编程基础（1年+ C/C++）的开发者。

### 1.9.1 筑基阶段：理解底层原理（3-6个月）
...
### 1.9.2 源码攻坚阶段：解剖标准库（6-12个月）

#### 1.9.2.1 glibc 内存分配（ptmalloc2）
...
- **源码调试**：跟踪 `std::mutex::lock()` 到 `pthread_mutex_lock`，再到内核 `futex`：
...
### 1.9.3 实战进阶：自定义与优化（6个月+）

#### 1.9.3.1 实现简化版 ptmalloc
...
1. **从简单案例入手**：先理解 `malloc(1)` 的分配逻辑，再扩展到复杂场景。
2. **动手修改源码**：编译自定义的 glibc 版本，观察修改 `M_TRIM_THRESHOLD` 对内存释放的影响。
3. **关注边界条件**：如内存不足时 `malloc` 的返回值、多线程同时释放同一块内存的竞态。
4. **对比不同实现**：分析 musl libc 与 glibc 在内存分配上的差异（musl 更轻量化）。
5. **结合操作系统**：学习内核内存管理（如 slab 分配器），理解用户态与内核态的交互。
