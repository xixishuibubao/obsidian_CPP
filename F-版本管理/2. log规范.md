...
约定式提交：`<type>[scope]: <description>`，让历史可读、可自动生成 CHANGELOG、对齐 SemVer。

## 【核心精简提纲】

### 记忆主干

格式：`<类型>[可选范围]: <描述>` + 可选正文 + 可选脚注。`feat`→MINOR、`fix`→PATCH、含 `BREAKING CHANGE` 或 `!`→MAJOR。本 vault 与 Agent 协作均要求此格式。

### 核心要点清单

- **核心 type**：`feat`（新功能）、`fix`（修 bug）
- **常用 type**：`docs` `refactor` `perf` `test` `build` `ci` `chore` `style`
- **破坏性变更**：`feat!:` 或脚注 `BREAKING CHANGE: …`
- **范围 scope**：`feat(parser):` 标明影响模块
- **与 SemVer**：fix→PATCH，feat→MINOR，BREAKING→MAJOR
- **本仓库习惯**：`docs(目录): 简体中文描述`；推远端前 squash 保持 log 整洁

### 学习路径

1. 记住 `feat` / `fix` / `docs` 三板斧
2. 练写带 scope 的单行 message
3. 了解 `!` 与 `BREAKING CHANGE` 脚注
4. 配合 [[F-版本管理/4. 分支策略|分支策略]] 与 squash 工作流

### 关键结论 / 流程

```
一次改动 → 选 type → 写 scope（可选）→ 动词开头描述
推远端前 → rebase -i / squash → 合并为一条规范 commit
```

## 【完整拓展详情】

官方规范：[Conventional Commits 1.0.0（中文）](https://www.conventionalcommits.org/zh-hans/v1.0.0/)
...
注：`!`需紧跟类型或范围后（如`feat!`、`feat(api)!`），用于明确标识破坏性变更，替代`BREAKING CHANGE`脚注脚注的写法，更简洁直观。
...
5. 描述字段必须直接跟在 <类型>(范围) 前缀的冒号和空格之后。 描述指的是对代码变更的简短总结，例如： _fix: array parsing issue when multiple spaces were contained in string_ 。
    
6. 在简短描述之后，可以编写较长的提交正文，为代码变更提供额外的上下文信息。正文必须起始于描述字段结束的一个空行后。
    
7. 提交的正文内容自由编写，并可以使用空行分隔不同段落。
    
8. 在正文结束的一个空行之后，可以编写一行或多行脚注。每行脚注都必须包含 一个令牌（token），后面紧跟 `:<space>` 或 `<space>#` 作为分隔符，后面再紧跟令牌的值（受 **[git trailer convention](https://git-scm.com/docs/git-interpret-trailers)** 启发）。
...
12. 包含在脚注中时，破坏性变更必须包含大写的文本 `BREAKING CHANGE`，后面紧跟着冒号、空格，然后是描述，例如： _BREAKING CHANGE: environment variables now take precedence over config files_ 。
    
13. 包含在 <类型>(范围) 前缀时，破坏性变更必须通过把 `!` 直接放在 `:` 前面标记出来。 如果使用了 `!`，那么脚注中可以不写 `BREAKING CHANGE:`， 同时提交信息的描述中应该用来描述破坏性变更。
    
14. 在提交说明中，可以使用 `feat` 和 `fix` 之外的类型，比如：_docs: updated ref docs._ 。
...
若使用 squash 工作流，普通提交者可自由提交，合并时维护者需按规范整理为`type(scope): description`格式（如`fix(parser): handle empty input`）。
...
- [[F-版本管理/1. git|Git 基础]] — `commit`、`rebase -i` 整理历史
- [[F-版本管理/4. 分支策略|分支策略]] — Squash and Merge 与规范 message
- [[E-AI与Agent协同开发/02.工作流与方法论/1. AI-Agent 协同开发（Vibecoding 最佳实践）|Vibecoding]] — Agent 协作中的提交策略