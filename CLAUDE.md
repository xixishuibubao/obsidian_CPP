...
H-语言与标记/           ← Markdown · CSS · XML · PlantUML · UML · INI · YAML · 英语
```
...
- **主 Agent**：Claude Code（本文件 + `.claude/instructions/` 为权威来源）
- **Cursor / 其他 Agent**：须先对齐上述规则与 Claude Code 项目记忆；细则见 [05-agent-coordination.md](.claude/instructions/05-agent-coordination.md)
...
6. **本文件保持简洁**（150~180 行），接近上限时拆分至 `.claude/instructions/` 子文件
...
| Command | 用途 |
|---------|------|
| `/ingest-note` | 笔记摄入：清洗原始文本 → 格式化 Markdown → 自动分类 → 更新索引 → 删除源文件 → 提交 |
