...
# 4. `bootloader & app`
## 4.1 定位编译
- `KeilAC5`的`__attribute__((at(0x08008000)))`实现定位编译
- `KeilAC6`的`__attribute__((section(".ARM.__at_0x08008000")))`
- `gcc`的`__attribute__`并不兼容这两种写法
- 常用于定位`flag`到`flash`，作为长久标记。
```c++
   // ================= 核心修复：确保编译器版本宏被正确识别 =================
   // 兼容Keil AC5/AC6，避免版本宏未定义导致的报错
   #if !defined(__ARMCOMPILER_VERSION)
   // 若未识别到ARM编译器，默认按AC6处理（或根据需求改为#error）
   #define __ARMCOMPILER_VERSION 6000000
   #endif
   ...
   // 修复：增加一层宏封装，解决嵌套宏参数字符串化问题
   #define _STRINGIFY(x) #x
   #define _TOSTRING(x) _STRINGIFY(x)  // 双层封装，支持宏参数展开
   
   // 方案 A: 快速兼容模式 (无需修改 .sct)
   // 修复：加used属性防优化，同时支持宏参数（如PLACE_AT(DFU_ADDR)）
   #define PLACE_AT(addr)  __attribute__((used, section(".ARM.__at_" _TOSTRING(addr))))
   
   // 方案 B: 标准工程模式 (推荐用于复杂项目)
   #define PLACE_IN_SECTION(sec_name)  __attribute__((used, section(_TOSTRING(sec_name))))
   
   #elif defined(__ARMCOMPILER_VERSION) && (__ARMCOMPILER_VERSION >= 5000000)
   /* ================= AC5 (Arm Compiler 5) ================= */
   // AC5兼容：支持_AT关键字，同时兼容AC6的段名写法
   #define PLACE_AT(addr)  __attribute__((at(addr), used))
   #define PLACE_IN_SECTION(sec_name)  __attribute__((section(sec_name), used))
   
   #else
   /* ================= 其他编译器 (如GCC) ================= */
   #error "Unsupported compiler! Only Keil AC5/AC6 are supported."
   #endif
```
## 4.2 `C & C++`混合编译
- 对于共用的`.cpp`文件中的函数要使用`extern "C"{ func}`包裹
- 在`.h`使用要额外使用`#ifdef`,否则混编的时候`c`编译器会报错
```c++
   #ifdef __cplusplus 
   extern "C" { 
   #endif 
...
```
## 4.3 `boot_to_app`
1. `boot`跳转的时候要关闭中断，`__disable_irq();`,且关闭后不可以使用`HAL_DELAY();`,因为`HAL_DELAY();`依赖`systick中断`
2. `APP`开始的时候要`__enable_irq();`,打开被`boot`关闭的中断
3. `__enable_irq();`,本身是无害的,重复打开也不会异常 
   - 推荐在`boot`和`APP`开始都打开
4. `APP`要在开始的时候设置偏移的`VEC`中断向量表,若在`boot`跳转前设置则可不必重复设置
5. `keil`的魔术棒->`C/C++(AC6)`编译选项->`Misc Controls`是添加用户编译选项(等价于在`gcc`后追加编译设置选项) 
   - `-include xxx.h`可以给全部文件包含某个头文件
   - 多个之间用空格隔开,`-include xxx1.h -include xxx2.h`,这里的头文件都是以`"xxx.h"`这种`" "`的用户库的方式引入的
6. `MCU`直接重复初始化系统时钟会导致异常
   - `boot`和`app`的时钟要么只配置一次,要么`app`要`deinit_RCC`复位之后,再配置
```c++
// APP入口函数（Boot跳转后执行的第一个函数）
void App_Entry(void)
{
  // 1. 关闭所有中断（避免时钟切换时被中断打断）
  __disable_irq();

  // 2. 重置时钟系统（可选，彻底清空Boot的时钟配置）
  HAL_RCC_DeInit();

  // 3. 重新初始化APP的时钟（比如配置为100MHz HSE）
  SystemClock_Config();//设置系统时钟

  // 4. 重新初始化HAL库（时钟变了，HAL_Init需要重新执行）
  HAL_Init();
...
## 4.4 `mircolib`
1. 单片机直接重定向`fputc`使用`printf`依赖`mircolib`
   - `mircolib`不支持很多`C++`特性
   - `printf`的流程如下:
     1. 用户调用`printf`
     2. 内部调用`vprintf`进行格式化,`vprintf`和`vsprintf`是同系列函数
     3. 调用`fputc`输出,`MCU`需要重定向`fputc`使用
     4. 若`MCU`不用`mircolib`而使用`printf`会进入多个断点,原因是`MCU`不存在许多系统IO调用的能力
2. 使用`vsprintf`代替`printf`实现自定义的硬件格式化输出
   - 先输出到缓冲区再转到通讯口输出
```C++
#include <stdarg.h>
#include <stdio.h>
...
int myprintf(const char *format, ...) {
    ...
    // 3. 核心：用vsprintf解析格式化字符串到缓冲区
    //    vsprintf返回写入的字符数，自动处理所有格式化规则
    int len = vsnprintf((char *) buf, 256, format, args);
    va_end(args);
...
// 带格式化标签的myprintf
void myprintf_with_format(const char *tag, const char *fmt, ...) {
    char log_buf[256] = {0};
    char content_buf[128] = {0};
    memset(log_buf, 0, sizeof(log_buf));
    memset(content_buf, 0, sizeof(content_buf));

    va_list args;

    // 1. 标签用%-12s（12是偶数，适配6个中文，足够所有标签）,%-12s是对%s的格式化占位输出,对数值固定占位输出同理
    snprintf(log_buf, sizeof(log_buf), "[BOOT] %-12s: ", tag);

    // 2. 内容单独处理，避免格式化符冲突
    va_start(args, fmt);
    vsnprintf(content_buf, sizeof(content_buf), fmt, args);
    va_end(args);

    // 3. 拼接内容（strncat防止溢出）
    strncat(log_buf, content_buf, sizeof(log_buf) - strlen(log_buf) - 1);
    strncat(log_buf, "\r\n", sizeof(log_buf) - strlen(log_buf) - 1);

    // 4. 用你之前正常的app_log输出（底层串口没问题）
    myprintf("%s", log_buf);
}
```
