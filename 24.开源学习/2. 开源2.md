...
> 纯C/C++ 后端/后台类项目推荐，涵盖高并发、存储引擎、分布式系统、编译器、加密库等领域。

## 1. 高性能网络库：libhv（C/C++混合）
