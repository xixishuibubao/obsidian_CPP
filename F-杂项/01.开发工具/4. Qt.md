...
- 使用信号函数：
  ...
  2. Qt4/5使用的方法不检查编译错误。
     `connect(sendobj*，SIGNAL(sendFn(形参列表)),recvobj*,SLOT(recvFn(形参列表)));`,
...
- Qt的函数命名是***大驼峰***，信号和槽依赖于`QObject`类，`connect`是`QObject`的静态方法。
---
- Qt键盘鼠标事件处理：
...
使用 Qt 目录下的 mingw（不带 tools 的那个）中的 `windeployqt.exe`，可将缺失的 DLL 复制到项目目录：

```bash
windeployqt.exe <可执行程序名>
```

解决 CLion 编译后运行异常退出的问题（通常是因为缺少某些 Qt 运行时库）。
