#!/usr/bin/env bash
# vault-audit.sh 鈥?Obsidian vault mechanical audit
  ...
  while IFS= read -r f; do
    ...
    while IFS= read -r line; do
      ...
      if ! resolve_wikilink "$f" "$target"; then
        add_issue P0 M1 "鏂摼 wikilink: $f 鈫?[[$target]]"
      fi
...
check_m2() {
  ...
  while IFS= read -r f; do
    ...
    while IFS= read -r img; do
      ...
      if [[ ! -f "$img" ]]; then
        add_issue P0 M2 "鍥剧墖鏂摼: $f 鈫?$img"
      fi
...
check_m3() {
  ...
  while IFS= read -r line; do
    ...
    if [[ "$actual" != "$expected" ]]; then
      add_issue P1 M3 "绱㈠紩婕傜Щ: $dir 鏂囨。=$actual 绱㈠紩=$expected"
    fi
...
check_m5() {
  ...
  while IFS= read -r f; do
    ...
    [[ -z "$first" ]] && continue
    if [[ ! "$first" =~ ^#\ 1\. ]]; then
      add_issue P1 M5 "棣栬涓€绾ф爣棰橀潪 # 1.: $f 鈫?$first"
    fi
  done < <(note_files | sort -u)
}
...
check_m6() {
  ...
  while IFS= read -r f; do
    ...
    while IFS= read -r linenum; do
      add_issue P1 M6 "鏃犳爣娉ㄤ唬鐮佸潡锛堝紑 fence锛? $f:$linenum"
  ...
  for dir in A-*/* B-*/* C-*/* D-*/* E-*/* F-* G-*; do
    ...
    while IFS= read -r f; do
      ...
      for existing in "${nums[@]:-}"; do
        [[ "$existing" == "$n" ]] && add_issue P2 M7 "搴忓彿鍐茬獊: $dir/$bn (N=$n 閲嶅)"
      done
      nums+=("$n")
...
check_m8() {
  ...
  for f in "${!inbound[@]}"; do
    ...
    [[ "${inbound[$f]:-0}" -eq 0 && "${outbound[$f]:-0}" -eq 0 ]] && \
      add_issue P2 M8 "瀛ょ珛绗旇: $f (0 鍏ラ摼 0 鍑洪摼)"
  done
}
...
check_m9() {
  ...
  for kw in "${STALE_KEYWORDS[@]}"; do
    while IFS= read -r hit; do
      ...
      f="${hit%%:*}"
      [[ "$f" == *'.claude/'* ]] && continue
      add_issue P1 M9 "闄堟棫鍏抽敭璇?'$kw': $hit"
...
check_m10() {
  ...
  while IFS= read -r f; do
    ...
    while IFS= read -r _; do
      add_issue P1 M10 "琛ㄦ牸鍐?wikilink 鍒悕: $f"
...
if [[ "$JSON_OUT" == true ]]; then
  ...
  echo "P0: $P0  P1: $P1  P2: $P2  鍚堣: $((P0 + P1 + P2))"
  echo ""
