---
name: quick-commit
description: 分析 git status/diff（含 AI 与 Obsidian 手动编辑）→ 生成逐文件 Conventional Commit → 用户确认后提交。Use when 用户说提交/commit 或 /quick-commit
---
...
- `.claude/instructions/01-repo-structure.md：E-AI 模块笔记计数 N→N+1，目录描述追加 Skill 总览条目
...
2. 换行符统一：
   ...
   git add --renormalize .
   ```
3. 对 `git diff --cached --name-only` 中的 `.md` 执行 [06-continuous-review.md](../../instructions/06-continuous-review.md) 7 项增量审查；机械问题先修复再提交
...
当 body 较长时，在 **Git Bash** 下可用 heredoc；PowerShell 回退时用多个 `-m` 传 body：
...
- **Commit message 必须详细**：每个文件至少一条 bullet point；格式详见 [03-git-workflow.md](../../instructions/03-git-workflow.md) §B
