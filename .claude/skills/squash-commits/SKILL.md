---
name: squash-commits
description: 推送前合并本地 commits，自动识别 obsidian-git 备份提交并 squash。Use when push 前整理历史、清理 wip/backup 或 /squash-commits
---
...
```bash
git log --oneline origin/main..HEAD    # 本地未推送 commits
# 若 origin/main 不存在，则 git log --oneline -10
```
...
**方法 A** 适合：连续微提交 → 合并为 1 个。**终态 message 必须**含逐文件 body，格式见 [03-git-workflow.md](../../instructions/03-git-workflow.md) §B：
...
```
  ...
  输入新的 commit message（含 header + body bullet，留空使用第一个提交的 message）:
```

**方法 B** 适合：有意义的多个提交想保留、部分需要合并。

> **Windows**：`GIT_SEQUENCE_EDITOR` + `sed` 须在 **Git Bash** 中执行；PowerShell 会话请回退手工 `git rebase -i origin/main`（无远程时用 `HEAD~N`）。
...
```
  ...
  pick e4f5g6h fix: 调整命名          ← 保留或按需 fixup
  fixup b2c3d4e wip: 临时保存          ← 自动 fixup（备份提交）
  pick h7i8j9k docs: 修复 typo        ← 保留或按需 fixup
```
...
git commit -m "type(scope): header" -m "- 文件1：改动\n- 文件2：改动"

# 或方法 B（Git Bash）
GIT_SEQUENCE_EDITOR="..." git rebase -i origin/main
# origin/main 不存在时：git rebase -i HEAD~N
```
...
- hook 拒绝时新建 commit，勿 amend（除非用户明确要求）
