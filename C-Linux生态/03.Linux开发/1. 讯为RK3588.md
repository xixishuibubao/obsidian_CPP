...
// 卸载函数必须是 void *()
static void myexit() { /* 内核退出函数，卸载时自动触发 */
}

module_init(myinit);
module_exit(myexit);
MODULE_LICENSE("GPL"); // 开源许可，必须
/* 以下是非必须，会被保存到 modinfo 里
...
# 加了 export 后面 make 的位置可以不用使用这两个变量，否则需要使用
...
all:
    ...
    # make xxx modules 是编译内核的关键，其他的都是参数设置
    # -C 改变目录到 KDIR（内核目录）M=实际要编译的模块文件在的目录
...
2. 交叉编译完之后使用 `scp` 传输到开发板，在开发板终端：  
   `scp 远程侧用户名@ip:带绝对路径的文件名 本地保存目录`
3. 编译完成后的加载与卸载：
...
1. `make menuconfig` 是一种通用的图形化配置方法，使用空格来切换选项状态。
   - `[ ]` 代表有两种模式，`[*]` 和 `[ ]`，选中和不选中，对应编译和不编译。
   - `< >` 代表有三种模式，`[M]` `[*]` `[ ]`，对应将驱动编译为模块、编译进内核、不编译。
   - `( )` 内部只可以填写十六进制或字符串。
...
1. 上面是增加一个自定义的 `helloworld` 驱动编译进内核的操作。除了在 `helloworld` 文件夹下创建这样的 `Kconfig` 文件，还要创建对应的 `Makefile` 如下：

```Makefile
obj-$(CONFIG_helloworld) += helloworld.o
# CONFIG_helloworld 正是由 Kconfig 生成的一个变量
```

然后退回到上一级目录。这里我们假设它是一个 `drivers/char/` 类型的驱动，所以要修改 `/char` 目录下的 `Kconfig` 文件，使用 `source "drivers/char/helloworld/Kconfig"` 引入。`source` 以文件内容直接引入（类似 `#include`）。
