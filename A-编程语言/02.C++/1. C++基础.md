...
C++ OOP、类语义（Rule 0/3/5、继承/多态）、模板与 C++11 特性的教程型速查。

## 【核心精简提纲】

### 记忆主干

**封装/继承/多态 · class/struct 仅默认访问不同 · Rule 0/3/5 管拷贝/移动/析构 · 虚析构+override · RAII/PIMPL/RTTI · 模板泛化/特化/CRTP · C++11 移动/智能指针/lambda**
...
- 模板：定义放头文件；类可偏/全特化；函数无偏特化靠重载；最优匹配 > 特化 > 泛型
...
| §3 模板 | 泛型/特化、CRTP、模板函数、decltype |
| §4 C++11 | 右值引用、移动、智能指针、lambda、并发原语 |

### 交叉引用

- [[A-编程语言/02.C++/2. 代码规范|C++ 代码规范]]
- [[A-编程语言/02.C++/4. template|template 专题]]
...
- class 默认权限是 私有， struct 默认权限是 公有。
...
> const 成员和引用成员必须使用初始化列表初始化：`ClassName() : const_member(val1), ref_member(val2) {}`。普通成员也可以这样初始化，但 const 和引用只能这样初始化。
...
- 前向声明只保证编译通过，此时不能访问类成员
- 一般在模板定义时使用
- 前向声明只适合数据类型（结构体、类、联合体、枚举），不适合变量
...
PIMPL（Pointer to Implementation）是一种避免暴露实现细节、减少编译依赖的 C++ 惯用法。详细介绍及 8 种实现方案参见：
...
<2>dynamic_cast 动态转换，主要转换父类与子类指针，转化失效
...
- 允许定义基础数据类型
- 具有类一样的作用域，不会造成同名冲突
- 使用时必须强制类型转换：`static_cast<int>(value)`

## 4.3 引用叠加原则

当对一个对象双层引用：有 `&` 则 `&`，全 `&&` 则 `&&`。

`&&` 是右值引用，旨在避免无意义的拷贝，多用于数据转移（移动语义）。