> C++ 类中的模板用法（泛型、特化、CRTP）参见：[[02.C++语法与要点/1. C++基础#3. 模板|C++ 基础 — 模板]]
...
模板是 C++98 引入的，模板元可以理解为对模板的一种特殊使用，可以在编译时就获取需要的返回值。
...
`<>` 内必须是确定的值，不能是运行中获取的动态值。

使用 `enum` 是因为枚举可以在编译期就确定值，使用 `static const` 也可以但不推荐：
